...
module('Integration | Component | list-filter', function(hooks) {
  ...
  test('should update with matching listings', async function (assert) {
    ...
    this.set('filterByCity', (val) =>  {
      ...
      // return { 
      //   query: val,
      //   results: ITEMS.filter(item => item.city.toLowerCase().includes(val))
      // }
    });
    ...
    const beginFilterCount = filterCount
    await triggerKeyEvent(this.element.querySelector('.list-filter input'), "keyup", 83);
    assert.equal(filterCount - beginFilterCount, 1, 'before adding a character to the input, filter has been invoked 1 more');
